...
import { getMyCommunityPosts } from "@/lib/content";
...
export default async function AccountPage() {
  ...
  const posts = await getMyCommunityPosts(session.user.id);
        ...
        <div className="grid gap-6">
          <form action={updateProfile} className="ec-panel ec-accent-trusted rounded-lg p-5">
            ...
            <div className="mt-4 grid gap-4">
              <label className="grid gap-2 text-sm font-bold text-white/80">
                Display name
                <input name="name" defaultValue={session.user.name || ""} className="rounded-md border border-white/10 bg-black px-3 py-2 text-white outline-none focus:border-[color:var(--ec-green)]" />
              </label>
              <label className="grid gap-2 text-sm font-bold text-white/80">
                Avatar URL
                <input name="avatarUrl" defaultValue={session.user.avatarUrl || ""} className="rounded-md border border-white/10 bg-black px-3 py-2 text-white outline-none focus:border-[color:var(--ec-green)]" />
              </label>
              <label className="grid gap-2 text-sm font-bold text-white/80">
                Bio
                <textarea name="bio" className="min-h-28 rounded-md border border-white/10 bg-black px-3 py-2 text-white outline-none focus:border-[color:var(--ec-green)]" />
              </label>
          ...
          <form action={createCommunityPost} className="ec-panel ec-accent-community rounded-lg p-5">
            ...
            <div className="mt-4 grid gap-4">
              ...
              <label className="grid gap-2 text-sm font-bold text-white/80">
                Image URL
                <input name="imageUrl" type="url" className="rounded-md border border-white/10 bg-black px-3 py-2 text-white outline-none focus:border-[color:var(--ec-violet)]" />
              </label>
              <label className="grid gap-2 text-sm font-bold text-white/80">
                Image alt text
